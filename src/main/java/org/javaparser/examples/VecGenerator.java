...
public class VecGenerator {
    ...
    MutableNetwork<Node, String> CFG;
    ...
    private ArrayList<Integer> getVecOfNode(Node node) {

        ArrayList<Integer> vec = new ArrayList<>(8);
        int outDegree;
        int inDegree;
        int NOMethodCalled;
        int isControlNode;
        ...
        int hasRelationOp;
        int isBinaryExpr;
        outDegree = CFG.outDegree(node);
        inDegree = CFG.inDegree(node);
...
//        // 检测出度
//        System.out.println(node.getRange().get().begin.toString()+inDegree);
//        // 检测入度
//        System.out.println(node.getRange().get().begin.toString()+outDegree);
//        // 检测方法调用数目
//        System.out.println(node.getRange().get().begin.toString()+NOMethodCalled);
//        // 检测控制语句判断
//        System.out.println(node.getRange().get().begin.toString()+isControlNode);
//        // 检测声明语句判断
//        System.out.println(node.getRange().get().begin.toString()+isDeclaration);
    ...
    private String getTail(String s){
        return s.substring("class com.github.javaparser.ast.stmt.".length());
    }
}
